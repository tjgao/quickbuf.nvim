...
local function apply_highlights(all_highlights)
  ...
  for line_idx, line_highlights in ipairs(all_highlights) do
    for _, item in ipairs(line_highlights) do
      vim.api.nvim_buf_add_highlight(M.buf, ns, item.hl, line_idx - 1, item.start_col, item.end_col)
    end
  end
end
...
local function apply_keymaps(items, labels_for_items, ctx)
  ...
  if config.values.alternate_key and config.values.alternate_key ~= "" then
    vim.keymap.set("n", config.values.alternate_key, function()
      ...
      if alternate > 0 and alternate ~= (ctx and ctx.source_bufnr or -1) and vim.api.nvim_buf_is_valid(alternate) and vim.bo[alternate].buflisted then
        pick(alternate)
      else
  ...
  if picker_keys.toggle_pin_key and picker_keys.toggle_pin_key ~= "" then
    vim.keymap.set("n", picker_keys.toggle_pin_key, toggle_current_pin, { buffer = M.buf, nowait = true, silent = true })
  end
...
function M.open(opts)
  ...
  for i, item in ipairs(shown) do
    if not (config.values.alternate_without_label and item.alternate and config.values.alternate_key and config.values.alternate_key ~= "") then
